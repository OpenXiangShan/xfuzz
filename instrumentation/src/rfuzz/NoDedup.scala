...
class NoDedupTransform extends Transform with DependencyAPIMigration {
  ...
  def countInsts(circuit: Circuit): Map[String, Int] = {
    ...
    def onMod(mod: DefModule): DefModule = mod.mapStmt(onStmt)
    circuit.mapModule(onMod)
    counts.toMap
  }
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    // Check counts
    val duplicated = counts.filter(_._2 != 1).map(_._1).toSeq
    ...
    val modNames = state.circuit.modules.filterNot(m => duplicated.contains(m.name)).map(m => ModuleName(m.name, circuitName))
