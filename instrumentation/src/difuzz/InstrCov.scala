...
class InstrCov(mod: DefModule, mInfo: ModuleInfo, extModules: Seq[String], val maxStateSize: Int = 20, val covSumSize: Int=30) {
  ...
  def annotate(circuit: String): (DefModule, Seq[CoverPointAnnotation]) = {
    mod match {
      case m: Module =>
        val ns = Namespace(m)
        val defNodes = allOffsets.map(_._1).map {
          case r: DefRegister =>
            DefNode(Utils.get_info(r), ns.newTemp, WRef(r))
          case e: Expression =>
            DefNode(NoInfo, ns.newTemp, e)
          case x => throw new Exception(s"Unsupported node type: ${x.getClass.getSimpleName}")
        }
        val (stmt, anno) = defNodes.map{n =>
          val named = ComponentName(n.name, ModuleName(m.name, CircuitName(circuit)))
          val anno = Seq(CoverPointAnnotation(named, CoverInfo("control")))
            (n, anno)
        }.unzip
        (m.copy(body = Block(m.body +: stmt)), anno.flatten)
      case m => throw new Exception(s"Unsupported module type: ${m.getClass.getSimpleName}")
    }
  }
