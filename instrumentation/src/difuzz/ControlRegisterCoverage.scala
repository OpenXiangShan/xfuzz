...
import firrtl._
import firrtl.ir._
import firrtl.passes.{InferTypes, ResolveFlows, ResolveKinds, LowerTypes}
...
class ControlRegisterCoverage extends Transform with DependencyAPIMigration {
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (m <- circuit.modules) {
      val gLedger = new GraphLedger(m).parseModule
    ...
    val hierWriter = new PrintWriter(new File(s"${instrCircuit.main}_hierarchy.txt"))
