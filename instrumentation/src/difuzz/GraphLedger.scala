...
// graphLeger
// 1) Generate graph which consists of Statements (DefRegister, DefNode, DefMemory, DefInstance, Port)
...
class GraphLedger(val module: DefModule) {
  ...
  */
  ...
  def findNode(s: FirrtlNode): Unit = {
    ...
    if (Set("DefInstance").contains(s.getClass.getSimpleName))
  ...
  def findEdge(n: Node, sinks: ListBuffer[String])(s: Statement): Unit = {
    s match {
      ...
      case _ => () // Port, DefWire, DefMemory, DefInstance
    }
  ...
  //Find first sources (Port, DefRegister, DefMemory, DefInstance).
  ...
  def getInstances: Set[DefInstance] = {
    defInstances.map {
      case inst@DefInstance(_, _, _, _) => inst
      case inst => throw new Exception(s"${inst.serialize} is not DefInstance class\n")
    }.toSet
  }
