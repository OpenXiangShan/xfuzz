...
import sic.passes.{AliasAnalysis, KeepClockAndResetPass}
import xfuzz.{CoverInfo, CoverPointAnnotation, CoverPointTransform, CoverType}
...
object ToggleCoveragePass extends Transform with DependencyAPIMigration {
  ...
  override def optionalPrerequisiteOf = Seq(Dependency[CoverPointTransform])
  ...
  private case class ModuleCtx(
    ...
    en:        ir.Reference,
    clk:       ir.Expression,
    cover:     mutable.ListBuffer[CoverPointAnnotation])
  ...
  private def onModule(m: ir.DefModule, c: CircuitTarget, annos: Annos, ignore: Set[String],
    ...
    m match {
      case mod: ir.Module if !ignore(mod.name) =>
        ...
        if (signals.isEmpty) { (mod, List(), None, Seq()) } else {
          Builder.findClock(mod, logger) match {
            case None => (mod, List(), None, Seq())
            case Some(clock) =>
      ...
      case other => (other, List(), None, Seq())
    }
  ...
  private def addCover(signal: Signal, ctx: ModuleCtx): (ir.Statement, Seq[String]) = {
    val node = DefNode(NoInfo, ctx.namespace.newTemp, signal.ref)
    val anno = CoverPointAnnotation(ComponentName(node.name, ctx.m.toNamed), CoverInfo("toggle", CoverType.Multibit))
    ctx.cover.prepend(anno)

    if (Builder.enableVerificationIR) {
      ...
      val stmts = prevStmt ++ Seq(didToggleNode) ++ covers.map(_._2) :+ node
