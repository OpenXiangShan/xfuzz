...
object LineCoveragePass extends Transform with DependencyAPIMigration {
      ...
      case mod: ir.Module if !ignore(mod.name) =>
        Builder.findClock(mod, logger) match {
          case Some(clock) =>
            ...
            val body = addCover(bodyInfo, ctx, None)
            mod.copy(body = body)
          case None =>
            mod
        }
  ...
  private def onStmt(s: ir.Statement, ctx: ModuleCtx): (ir.Statement, Boolean, Seq[ir.Info]) = s match {
    case c @ ir.Conditionally(_, cond, conseq, alt) =>
      val truInfo = onStmt(conseq, ctx)
      val falsInfo = onStmt(alt, ctx)
      val doCover = truInfo._2 || falsInfo._2
      val stmt = c.copy(conseq = addCover(truInfo, ctx, Some((true, cond))), alt = addCover(falsInfo, ctx, Some((false, cond))))
  ...
  private def addCover(info: (ir.Statement, Boolean, Seq[ir.Info]), ctx: ModuleCtx, cond: Option[(Boolean, ir.Expression)]): ir.Statement = {
    ...
    if (!doCover) { stmt }
      ...
      val cover = if (Builder.enableVerificationIR) {
        Some(ir.Verification(ir.Formal.Cover, ir.NoInfo, ctx.clk, Utils.True(), Utils.True(), ir.StringLit(""), name))
      } else None
      if (cond.isDefined) {
        val node = DefNode(MultiInfo(infos), ctx.namespace.newTemp, cond.get._2)
        val named = ComponentName(node.name, ctx.m.toNamed)
        ctx.annos.prepend(CoverPointAnnotation(named, CoverInfo("line")))
        if (cover.isDefined) ir.Block(node, cover.get, stmt) else ir.Block(node, stmt)
      } else {
        if (cover.isDefined) ir.Block(cover.get, stmt) else stmt
      }
    }
  }
}
