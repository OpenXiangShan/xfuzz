...
object RegisterResetAnnotationPass extends Transform with DependencyAPIMigration {
  ...
  def findResetsInModule(m: ModuleTarget, annos: Seq[RegisterResetAnnotation]): Map[String, String] = {
    annos.filter(_.reset.module == m.module).flatMap { a =>
  ...
  private def onModule(m: ir.DefModule, c: CircuitTarget, annos: mutable.ListBuffer[RegisterResetAnnotation]): ir.DefModule = m match {
    case mod: ir.Module =>
  ...
  private def onStmt(s: ir.Statement, resets: mutable.ListBuffer[(String, String)],
    signalNames: mutable.HashMap[String, String], namespace: Namespace): ir.Statement = s match {
        ...
        case other =>
          signalNames.get(other.serialize) match {
            ...
            case None =>
              ...
              ir.Block(node,
                reg.copy(reset = ir.Reference(node.name, Utils.BoolType, NodeKind, SourceFlow))
              )
          }
      }
